...
- **Production reliability data (Section 4.1, Tables 2–3):** The V0 vs. V1 error rate comparison was collected from internal production logs during a 15-day parallel rollout. This data cannot be independently regenerated, but the architectural differences responsible for the improvements (co-located execution eliminating inter-pod HTTP communication failures) can be verified by code inspection.
- **LLM nondeterminism:** Benchmark results inherently depend on LLM inference behavior, which may vary across API versions and over time. We report results at specific SDK commits and model versions; reviewers should expect minor variance.
- **Cost considerations:** Full benchmark re-execution (Tier 4b) requires significant LLM API spend ($100–$1000 per benchmark). We recommend Tiers 1–3 and Tier 4a for validation: Tiers 1–3 together take under 15 minutes and cost under $3, while Tier 4a (downloading and inspecting historic evaluation artifacts from [https://index.openhands.dev](https://index.openhands.dev)) is free and allows verification of reported results without re-running experiments.
- **Docker requirements:** Docker is required for Tier 4b (benchmark re-execution with sandboxed execution) and optional for Tier 3. Tiers 1, 2, and 4a run without Docker.
- **SDK–Benchmarks compatibility:** Not every benchmarks commit is compatible with every SDK version. The benchmarks repo documents compatibility requirements. When reproducing, use the SDK commit pinned by the benchmarks submodule.
...
- [Artifact submission guidelines](https://ctuning.org/ae/submission.html)
- [Artifact reviewing guidelines](https://ctuning.org/ae/reviewing.html)
